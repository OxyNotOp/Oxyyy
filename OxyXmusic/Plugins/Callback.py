...
from OxyXmusic.Core.PyTgCalls import Queues
from OxyXmusic.Core.PyTgCalls.Converter import convert
from OxyXmusic.Core.PyTgCalls.Downloader import download
from OxyXmusic.Core.PyTgCalls.OxyXmusic import (join_stream, pause_stream,
...
from OxyXmusic.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from OxyXmusic.Decorators.admins import AdminRightsCheckCB
from OxyXmusic.Decorators.checker import checkerCB
from OxyXmusic.Inline import (audio_markup, audio_markup2, download_markup,
                          fetch_playlist, paste_queue_markup, primary_markup,
                          secondary_markup2)
from OxyXmusic.Utilities.changers import time_to_seconds
from OxyXmusic.Utilities.chat import specialfont_to_normal
from OxyXmusic.Utilities.paste import isPreviewUp, paste_queue
from OxyXmusic.Utilities.theme import check_theme
from OxyXmusic.Utilities.thumbnails import gen_thumb
from OxyXmusic.Utilities.timer import start_timer
from OxyXmusic.Utilities.youtube import get_m3u8, get_yt_info_id
...
async def play_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        for_t = 0
        for_p = 0
        for OxyNotOp in _playlist:
            _note = await get_playlist(user_id, OxyNotOp, type)
            title = _note["title"]
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        for OxyNotOp in _playlist:
            j += 1
            _note = await get_playlist(user_id, OxyNotOp, genre)
            title = _note["title"]
            duration = _note["duration"]
            msg += f"{j}- {title[:60]}\n"
...
async def del_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        await CallbackQuery.answer()
        for OxyNotOp in _playlist:
            await delete_playlist(user_id, OxyNotOp, genre)
    await CallbackQuery.message.reply_text(
